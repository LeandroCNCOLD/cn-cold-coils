...
public/data/catalogs/equipment.json  (padrão)

Lê a aba "480 modelos" (header na linha 1) e gera o arquivo JSON (ou TS).
Inclui geometria de aletado (condensador, evaporador e reaquecimento).

NUNCA inventa valores: se a célula estiver vazia ou contiver "-",
o campo correspondente fica omitido no JSON (ou `undefined` no TS).

Uso:
    python convert_catalog_ts.py <input.xlsx> <output.json>   # JSON (padrão)
    python convert_catalog_ts.py <input.xlsx> <output.ts>     # TypeScript (legado)
"""
...
def row_to_json(row: dict[str, object]) -> dict:
    out: dict = {}
...
def main(argv: list[str]) -> int:
    if len(argv) != 3:
        print("Uso: python convert_catalog_ts.py <input.xlsx> <output.json|output.ts>")
        return 2
    ...
    for _, raw_row in df.iterrows():
        ...
        if emit_json:
            rows_json.append(row_to_json(record))
        else:
    ...
    if emit_json:
        dst.write_text(json.dumps(rows_json, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"OK: {len(rows_json)} modelos escritos em {dst}. Ignorados: {skipped}.")
    else:
        header = (
