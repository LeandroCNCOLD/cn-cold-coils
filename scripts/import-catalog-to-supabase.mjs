...
function mapRow(item) {
  return {
    ...
    aplicacao:                item.application ?? item.aplicacao ?? null,
    familia:                  item.family ?? item.familia ?? null,
...
for (let i = 0; i < rows.length; i += BATCH) {
  ...
  if (!res.ok) {
    ...
    console.error(`\n❌  Lote ${i}–${i + batch.length} falhou: ${res.status} ${err.slice(0, 200)}`);
    errors += batch.length;
  } else {
